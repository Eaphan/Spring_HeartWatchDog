    ...
    <div class="main_container">
      ...
      <div class="right_col" role="main">
        <div class="col-md-3 col-sm-3 col-xs-3">
          <div class="panel panel-primary">
            ...
            <div class="panel-body">
              <div class="list-group">
                <a href="#" class="list-group-item">你好，${user.uname}</a>
                <a href="logout" class="list-group-item">退出</a>
              </div>
            </div>
          </div>
          <div class="panel panel-primary" id="onlineusers">
            <div class="panel-heading">
          ...
          <div class="panel panel-primary" id="onlinedoctors">
            <div class="panel-heading">
              <h3 class="panel-title">当前在线的私人医生</h3>
            </div>
            <div class="panel-body">
              <div class="list-group" id="doctors"></div>
            </div>
          </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        if ('WebSocket' in window) {
            websocket = new WebSocket("ws://localhost:8080/ws?uid="+${user.uid});
//            alert("当前浏览器支持websocket")
        } else if ('MozWebSocket' in window) {
            websocket = new MozWebSocket("ws://localhost:8080/ws"+${user.uid});
        } else {
            websocket = new SockJS("http://localhost:8080/ws/sockjs"+${user.uid});
        }
        ...
        websocket.onmessage = function(event) {
            ...
            }else if(data.from==-3){//上线消息
                if(data.text!="${user.uname}")
                {
                    $("#doctors").append('<a href="#" onclick="talk(this)" class="list-group-item">'+data.text+'</a>');
                    alert(data.text+"上线了");
                }
            }else if(data.from==-4){//下线消息
                if(data.text!="${user.uname}")
                {
                    $("#doctors > a").remove(":contains('"+data.text+"')");
                    alert(data.text+"下线了");
                }
            }
        };
        ...
        $.post("onlinedoctors",function(data){
            for(var i=0;i<data.length;i++)
                $("#doctors").append('<a href="#" onclick="talk(this)" class="list-group-item">'+data[i]+'</a>');
        });
